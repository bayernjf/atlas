...
> **状态**：题一「设计态内核前置项」（Capability input/outputSchema 声明 + 拓扑作用域索引 + L2 模板引用校验）**已于 2026-09-16 在 08 Phase 2 延伸区立项待落码**（范围以 08 立项记录为准）；其余提案（Schema 内核通用化、作用域 v2、多智能体任务总线、版本化灰度、交互模板、span 追踪）登记于 14 文档 D29-D34。文内新增依赖（AJV、Web Worker 等）、协同协议、版本/灰度模型仍为提案；落码前须按治理规则先走 10 文档 ADR + 08 计划记录 + 03 契约索引同步，不得直接视为已定选型或已生效契约。
