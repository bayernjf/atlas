...
| GET | /api/tasks | 任务信封查询（**M7 立项 2026-09-17 登记，08 M7 立项条，落码时生效；进程内 v1 无该端点**）：`?state=&assignee=&limit=`（新→旧），返回 `{items:[{taskId, runId, type, assignee, state, deadlineMs, attempt, result?}]}`；仅本租户 | task_envelope |
| GET | /api/tasks/{task_id} | 单任务详情（同上 M7 生效）：`{taskId, runId, idempotencyKey, type, assignee, payload, deadlineMs, state, result, attempt}`；跨租户/不存在 404；**人工升级不新增写端点**——升级复用 `POST /api/approvals/{token}/decision`（升级帧进 human_approval） | task_envelope |
