...
| GET | /api/alert-rule-templates | **docs/59 F-1（立项，read）**：内置只读告警规则模板目录列表 `{items:[{id,name,description,tags}]}`，投影**不含 config**；全局共享只读代码常量、不按租户分区、不受 reset 影响（照 /api/templates） | monitoring/rule_templates |
| GET | /api/alert-rule-templates/{id} | **docs/59 F-1（立项，read）**：完整 RuleTemplateMeta（含可直接 PUT rules 的完整 RuleConfig `config`）；未知 id 404。一键应用＝前端取 config 后复用 `PUT /api/monitoring/rules`（administer，全量替换、过 validate_rules），后端不另设写端点 | monitoring/rule_templates |

> **docs/59 F-2（立项，迁移 024）**：PG 档 `/api/monitoring/silences`、`/api/monitoring/on-call` 的状态与新建告警 `assignee` 由进程内 `OpsStore` 改为 PG 持久化（新表 `monitoring_silences`/`monitoring_oncall`，REST 形状不变、跨重启/跨实例保留、`suppressed_count` 落库）；内存档继续用进程内 `OpsStore`，行为不变。惰性升级 `escalated_at` 已由 docs/55（迁移 021）PG 化，本批不重做。
