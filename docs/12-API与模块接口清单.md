...
| POST | /api/graphs/{id}/compile | DSL → LangGraph 编译（08 7.1 W7-W8）。静态校验失败 422 体 `{"detail":[中文消息,...]}` 不变；**M2 起（已立项未开工）并列增机器可读定位侧车** `"locations":[{"index":number,"nodeId"?:"…","pointer"?:"/branches/0/expression"}]`——index 对齐 detail 下标、稀疏，pointer 为 RFC6901 相对该节点 config 根；图级错误无条目；契约见 03 `diagnostic`、06 §6.13，用例 U38。run/run/stream 经同一 compile_graph 路径，形状相同 | 02 Graph DSL |
