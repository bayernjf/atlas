...
def run_release_gate(*, graph_id: str, draft: dict, cases: list[RecordingCase],
    ...
    # D26 报告 v1（2026-09-18 立项、待落码）：gate.py 仍为纯函数；API 层每次跑完沉淀 ReleaseReport
    #   （release-gate→manual，publish gate→publish-gate，含 skipped/blocked），响应纯超集加 id

# src/atlas/recording/reports.py（D26 报告 v1，2026-09-18 立项、待落码；04 §5.11 末 / 03 release_report）
...
| POST | /api/graphs/{id}/release-gate | 【operate，M9 已落码 2026-09-18】发布前批量回放门禁（只跑门禁不发布，D26 部分取回）：对当前 latest 草稿逐例重放 `case.graph_id==id` 的录制用例（复用 replay.compare），返回 GateReport（D26 报告 v1 起纯超集加 `id`＝沉淀报告 rr-N）`{id,graph_id, target:"draft", total, passed, failed, skipped, blocked, cases:[{case_id,name,matches,replay_status,note?}]}`；total=0 时 skipped=true 不阻塞（明示未覆盖），total>0 任一不匹配 blocked=true；图不存在 404。**D26 报告 v1（2026-09-18 立项、待落码）起每次运行沉淀一条 ReleaseReport（trigger=manual，含 skipped）**；publish `{gate:true}` 另沉淀 trigger=publish-gate（通过/blocked 均沉淀，409 报告体带 id） | release_gate / release_report / recording_case |
| GET | /api/graphs/{id}/release-reports | 【read，D26 报告 v1，2026-09-18 立项、待落码】本图批量回放报告历史（倒序摘要）：`{items:[{id,graph_id,trigger,total,passed,failed,skipped,blocked,pass_rate,created_at}]}`，不含 cases；图不存在 404，跨租户 404 | release_report |
| GET | /api/graphs/{id}/release-reports/{rid} | 【read，D26 报告 v1，立项、待落码】报告详情（含 cases 逐例 ✓/✗/note）；报告不属于该图或不存在 404，跨租户不泄漏存在性 | release_report |
