...
class EventWaitBroker:
    ...
    def signal_key(self, event_key, payload) -> {released:int, queued:bool}: ...  # docs/55：广播释放同 key 全部 pending 返释放数；无有效 pending 则入 per-key 排队 ring（cap4/TTL3600）返 queued=true
...
# static（缺省）：seconds = int(config["durationSeconds"])             # 现状零改动
# dynamic：seconds = evaluate_expression(config["durationExpression"], context)
#   复用 atlas.graph.conditions 表达式引擎（{{路径}} 变量＋算术/白名单函数）
#   结果须为非 bool 数值、有限、1 <= round(seconds) <= 600，取 int(round(seconds))
#   求值异常（ConditionEvalError 等）或结果非数值/越界
#     -> WaitNodeFailure(code="WAIT_DURATION_INVALID")，run failed，不 sleep
```
...
### 3.20 到点时刻等待内部接口（durationMode=absolute v1；docs/50，2026-09-23 落码收口 12de72d）
...
| GET | /api/release-reports | 【read，docs/28 §2.4 已落码 2026-09-20，`89e21fc`】**跨图**用例集报告看板：query `limit`（默认 100、clamp 1-200、非整数 422），返回 `{items:[{id,graph_id,trigger,total,passed,failed,skipped,blocked,pass_rate,created_at}]}` 跨本租户全部图倒序、不含 cases；内存档 `ReportStore.list_all_summary`（ring 倒序切片），**PG 档自 docs/56（2026-09-24，`39c58bb`，迁移 022）起走 `recording/pg_reports.py` PgReportStore（不进 Repository 抽象的口径不变）**；viewer 可读 | release_report |
| GET | /api/schedules | 【read，**打包 N 已落码 2026-09-26**，docs/68 §2.4】列本租户的定时注册项（**登记来自发布派生，不是用户另填的表**：只有含 `schedule/cron` 触发节点的**已发布**版本才会出现在这里）。投影 camelCase 逐键＝`graphId`/`version`/`cron`/`enabled`/`createdAt`/`lastFiredAt`/`lastSkippedAt`/`skipCount`/`nextFireAt`，两档共用同一个 `schedule_projection`（`scheduling/models.py`）⇒ 内存档与 PG 档不可能给出两种形状。**所有时刻一律 UTC**，`nextFireAt` 是"严格晚于当下"的下一个槽（`*/5` 在 10:20 之后是 10:25） | schedule_record |
| POST | /api/schedules/{graph_id}/enabled | 【operate，同上】`{enabled: bool}` 开/关一条调度，**跨重启保留**、重发布不改写它（U888）。关掉只停**未来**触发，不取消在途运行（docs/68 §6.5）。该图没有调度 ⇒ 404 中文（含"只有已发布版本才会登记"的排障线索），跨租户同样 404 | schedule_record |
| POST | /api/schedules/{graph_id}/run-now | 【operate，同上】**联调与验收入口**：按登记的钉版立刻起一次后台运行，返回 `{runId, graphId, version}`。刻意**不写 `schedule_fires`、不占槽位**（U889 用"同一槽随后仍能认领成功"来钉这条），否则一次手工测试会吃掉本分钟的自动触发；`lastFiredAt` 仍更新（运营读的是"跑过没有"）。同图仍有 running/suspended ⇒ 409 中文；无调度 ⇒ 404 | schedule_record |
| POST | /api/schedules/cron-preview | 【read，同上，步 ⑤ 补】cron **预演**：body `{cron}` ⇒ `{valid, message, nextFireAt[3], timeZone:"UTC"}`。**合法与非法都回 200**（"这个串不对"是本题答案而不是 HTTP 错误），空串/缺串才 422 中文。不落库、不占槽位、不建调度。加它的原因是把后端那份唯一 cron 解释器借给画布用——前端复刻第二个解析器必然在"日与周取并集""7＝周日"这类边角上分叉，表现为字段看着合法、保存才 422 | schedule_record |
...
| GET | /api/monitoring/rules | 读取规则配置 RuleConfig：四内置规则（默认全开，连续阈值 3、窗口 20/最小样本 5/失败率 0.5）＋纯超集 `custom:[CustomRule]`（⑨，缺省 []）＋纯超集 `recovery_healthy_streak`（docs/55，默认 1、1-20 连续健康才自动恢复）、`recovery_cooldown_minutes`（docs/55，可空、1-10080 恢复冷却抑制重复 new 外发） | monitoring |
| PUT | /api/monitoring/rules | 全量替换规则配置；非法值（enabled 非 bool、阈值/窗口/样本非 1-200 整数、rate 非 0-1）422 中文聚合错误；**⑨ 起 `custom` 段同校验（cid 非空/同配置唯一/≤64、name 非空/≤50、severity 枚举、expression 经安全条件引擎静态校验且顶层须布尔，错误中文聚合 `custom[i].xxx：…`），旧配置无 custom 不 422；docs/55 起校验 recovery_healthy_streak 为 1-20 整数、recovery_cooldown_minutes 为空或 1-10080 整数，非法中文 422、缺省回落默认**；`/api/demo/reset` 恢复默认 | monitoring |
...
| PUT | /api/monitoring/silences/{silence_id} | **docs/60 G3（2026-09-24 落码 `9dfc593`，administer）**：编辑静默，body `{reason?:非空≤200, expires_at?:ISO8601}`（只改原因/到期，created_at/rule_id/graph_id/created_by 不可变）；不存在 404、已过期（expires_at≤now）409 SILENCE_NOT_ACTIVE、viewer 403；返更新后 Silence | monitoring/silences |
| GET | /api/monitoring/on-call | **docs/33 §5 ⑩（read）**：值班表，返 OnCallSchedule + 计算字段 `current`（空表为 null） | monitoring/silences |
| PUT | /api/monitoring/on-call | **docs/33 §5 ⑩（administer）**：设置轮值表；body `{members:[1-20 个非空串], rotationIntervalDays?:1-365 int｜null}`（去重保序、重置 index=0；**docs/60 G3（`9dfc593`，迁移 026）** rotationIntervalDays 非空启用读路径惰性按日轮换、null/缺省关闭，非法 422），数量/元素非法 422；返更新后 schedule + current（含 rotationIntervalDays/lastRotatedAt） | monitoring/silences |
...
> condition 节点（Phase 2 首版）运行结果写入 `outputs[condition_id] = {branch, target, evaluation:[{label,expression,result}], expression_errors:[string]}`（默认分支 `branch="__default__"`）；执行轨迹 messages 增一行 `condition-x: branch=… → target`。短路求值与 fail-safe 语义见 04 §5.2、06 §6.1。**docs/60 G1（2026-09-24 落码 `2b00bbb`/`9a2836a`）运行时错误码化**：condition 产出在 `expression_errors`（中文兜底，保留不删）之外**纯超集**增 `errorCode?:"CONDITION_EVAL_INVALID"` 与 `expressionErrorCodes?:(string|null)[]`（与各分支表达式等长、缺省 null，坏表达式位填 CONDITION_EVAL_INVALID）；前端 `lib/runtimeError.ts` 按 errorCode 映射 i18n（runtime.json zh/en）并据 expressionErrorCodes 定位到具体分支/字段，缺码回退中文 message。
>
...
| POST | /api/demo/reset | 【admin only】重置**调用方租户**的 Demo 数据（2026-09-16，04 §5.14：角色不足 403；作用域为本租户 graph/approval/debug/monitoring/message，录制与反馈按租户保留不清；跨租户数据不动）：本租户店铺——全局 demo 店铺恢复 5 笔种子退款单、清空已保存图与登录态、清空 pending 审批请求，Phase 1 种子客户体验，W10 后；Phase 2 第三项起清空进程内消息记录并重建内置 SQLite demo 订单库（全局共享模拟基础设施，仍随 reset 重建）——显式 `ATLAS_DATABASE_URL` 配置的外部库不被触碰；内置流程模板目录为代码常量，不受 reset 影响；录制用例为测试资产同样**不被 reset 清除**——其图已快照进用例，GraphStore 清空不影响回放；Phase 2 能力项起 reset 同时把全部**活动调试暂停按 stop 放行**（会话 cancelled + Event set），阻塞在 paused 的运行线程经 DebugStopped 收敛结束，不留悬挂线程；调试会话本身为进程内临时态，不构成需保留的数据；Phase 2 能力项（基础监控告警）起 reset 同时清空**本租户**监控运行记录与告警（ring、streak、Alert 列表）并把规则阈值恢复默认（运行计数不重置；运行时数据，同消息记录；重启本就清空，持久化随 11 S1/14 D28）。**M5 契约设计轮分层语义（2026-09-17，docs/24 §3.4）**：进程内后端行为不变；PG 档（M5b）＝truncate 本租户运行时表（graphs/runs/interruptions/iam_sessions/monitoring_*）并重建种子，`recordings`/`feedback`（persistent 档）保留不清。**M9 起 reset 同时清空本租户 RoutingStore（rollout 配置/状态/流量计数，进程内运行时态，memory/PG 两档同口径）**。**打包 N（2026-09-26，docs/68）起 reset 连带清空本租户 `schedules` 与 `schedule_fires`**——图已清而注册项还在，调度器就会对着一张不存在的图空转派发；两档同一口径（内存档清集合，PG 档删本租户行，U890b/U886e 分别钉住）。 | — |
| GET | /api/approvals | 列出当前 pending 审批请求（`{items:[{token, summary, approver, timeoutSeconds, node_id, graph_id, createdAt, cardTemplateId?}]}`，M8 起命中卡片附 cardTemplateId；**审批闭环批（2026-09-22）起附 createdAt**（pending 创建 epoch 秒，队列剩余时间计算用）；进程内单例，重启即失；Phase 2 第五项） | human_approval |
| GET | /api/approvals/decided | 已决审批只读历史（审计可见与审批结果补强批 2026-09-22，ecfa325/bc3f116，docs/37；**打包 H H2 已落码定稿 2026-09-25，`33a5998`/`528f6b1`/`e380f0b`，迁移 027，形状权威＝docs/61 §3**）：query `?limit=50`（clamp 1–200）；read 能力、**全部登录角色可用**、本租户分区；返 `{items:[{token, node_id, graph_id, summary, approver, createdAt, resolvedAt, decision, resolvedBy, comment, cardTemplateId?}], limit}`，按 resolved_at 倒序；不返回 card_context/notify_recipients/Event；**`createdAt` 自 H2 起为 UTC ISO-8601 串（原 float epoch 秒，破坏性变更，退化预案见 docs/61 §3.3）、新增 `resolvedAt` 同为 ISO**；持久化两档：内存 `InMemoryApprovalHistoryStore` ring 200，PG 档 `approval_history` 表跨重启与跨实例可见（**「reset/重启即失」口径自 H2 起只对内存档成立**，D20 挂起审批侧不受影响）。决策成功后各决策路径旁路 fail-safe 发决策结果邮件（`ApprovalNotifier.notify_decided`，异常仅 warning，不改 200/409 响应、重复决策不重发） | decided_approval |
...
| POST | /api/waits/events | 事件等待按 key 广播信号（D19 进程内 v1，2026-09-23 落码收口 docs/47；operate）：体 `{eventKey（必填 1-128、白名单 [A-Za-z0-9:_-]）, payload?: object}`（payload 序列化 ≤4096、顶层键 ≤50）；返 `{released: n, queued: bool}`（docs/55），同 key 多个 pending 全释放；无有效 pending 时 released=0、信号入 per-key 排队 ring（cap 4/键、TTL 3600s、登记时消费，仅同进程）queued=true（PG 单实例重启恢复后的 pending 同样可释放，docs/53；跨重启排队仍缓做）；eventKey 非法 422 WAIT_EVENT_KEY_INVALID、payload 超限 422 WAIT_EVENT_PAYLOAD_INVALID | wait_event |
| POST | /api/waits/{token}/signal | 事件等待直投信号（operate）：体 `{payload?: object}`（限制同上）；200 返 `{token, released: true, queued: bool}`（docs/55 纯超集加 queued）；未知/已取走 token 404 WAIT_TOKEN_NOT_FOUND；条目已 signaled 409 WAIT_ALREADY_SIGNALED | wait_event |
| GET | /api/waits | 列出本租户 pending 事件等待（viewer+）：`{items:[{token, eventKey, eventKeys?(多键时,docs/54), eventWaitMode?(any|all,docs/55), nodeId, graphId, timeoutSeconds, deadlineAt}]}`；多键时 eventKey 为首键、eventKeys 为全量、eventWaitMode 透传（纯超集，缺省 any）；PG 后端跨重启可见（docs/53，2026-09-23 立项：启动 restore 同 token/event_key 剩余超时；重启窗口内无进程接收的信号不排队，调用方需重试），进程内后端重启即失 | wait_event |
