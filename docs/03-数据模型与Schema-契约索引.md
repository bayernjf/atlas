...
```yaml
id: string                    # 唯一ID
type: enum[trigger, ai_decision, tool_call, condition, loop]
name: string
description: string
position: {x, y}
config:
...
nodes:                       # node_schema 节点列表；当前可编译类型：
                             #   trigger / ai_decision / tool_call / condition / loop（Phase 2 起），
                             #   其余类型校验拒绝；condition config 契约见 04 §5.2，loop config 契约见 04 §5.3
