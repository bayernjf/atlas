...
| `channel_binding` | **docs/38 立项 2026-09-22（真实渠道适配批，ADR T28，docs-only 契约先行）**；权威 docs/38 §3 + `src/atlas/channels/{base,shopify,registry,memory,pg,adapter}.py`＋`storage/pg.py` PgChannelStore（表 `channel_bindings`，迁移 015，reset 不清）。绑定按租户引用 T4 connection（`connection_id` 同租户唯一），连接被删不级联（置 status=error）；字段概览见本文末 `channel_binding` 段；5 端点见 12 文档。**2026-09-23 docs/39（ADR T29）起新增列 `webhook_subscriptions`（迁移 016，TEXT NOT NULL DEFAULT `'[]'`，JSON：`WebhookSubscription[]`＝`{topic, graph_id, enabled}`，topic ∈ orders/create·orders/updated·refunds/create，topic+graph_id 同绑定唯一、≤10）；reset 不清** | docs/38 §3（形状权威）；工具输入/输出见 docs/38 §1B，订单投影见 §1A；**webhook 订阅见 docs/39 §1B**
