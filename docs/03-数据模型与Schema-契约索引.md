...
| `decided_approval` | **审计可见与审批结果补强批已落码收口 2026-09-22（ecfa325/bc3f116/033619a，docs/37）**；`ApprovalBroker.list_decided` 只读投影＋`GET /api/approvals/decided`；`_Pending.notify_recipients`＋决策结果邮件；权威＝04 §5.6 补强批追加段；**打包 H H2 已落码定稿（2026-09-25，`33a5998`/`528f6b1`/`e380f0b`；形状权威＝[docs/61](61-编译诊断定位-审批历史PG-审计过滤分页-影子PG化-口径勘误v1批契约设计.md) §3，偏差见其顶部注记 ④）**：新增 `approval_history` 落库形状——新模块 `collaboration/history.py`（`ApprovalHistoryStore` Protocol：`record/list(limit=50) -> resolved_at 倒序 clamp 1-200/clear`＋`InMemoryApprovalHistoryStore` deque ring 200＋`PgApprovalHistoryStore`）、迁移 `027_approval_history`（表 `approval_history`：`tenant_id`、`token`、`seq BIGINT` 取 `storage_id_seq`、`node_id`、`graph_id`、`summary`、`approver`、`decision`、`resolved_by`、`comment` NULL、`card_template_id` NULL、`created_at` TEXT、`resolved_at` TEXT；PK `(tenant_id, token)`、索引 `(tenant_id, seq DESC)`；`002_storage.sql` 同步），`ApprovalHistoryEntry` 十字段同上（**created_at/resolved_at 为 UTC ISO-8601 TEXT**——`_Pending` 内存态仍持 `time.time()` float epoch，仅在写 history/出投影时经单一转换函数转 ISO）；`ApprovalBroker(history_store=None)` 缺省内存版、`registry.py:132` PG 档注入、reset 随 approval_broker 清本租户、跨重启与跨实例可见；`GET /api/approvals/decided`（read）签名与权限零改动，**投影纯超集增 `resolvedAt`，`createdAt` 由 float epoch 改 ISO 串（破坏性变更，退化预案见 docs/61 §3.3）**，`notify_recipients`/`card_context`/`action_id` 仍不入投影；**pending 挂起审批不动**（仍由 `interruption_frame`/`interruptions` 承担） | —（工程契约） |
...
| `message_send_params` | 04 / 四、工具/适配器组件 4.8 消息适配器（进程内消息服务）v1 契约（权威 blockquote；IM 群机器人 docs/51；**投递日志/退避 docs/56，2026-09-24 落码 1237a2e**）+ `src/atlas/message/{service,adapter}.py`（单能力 message/send；service 另持 DeliveryRecord ring 与 `_transmit` 重试；**webhook 出站 HMAC/IM markdown 富文本与 @人/多 URL 群发见 docs/58，2026-09-24 立项中**） | ### 4.8 消息适配器（进程内消息服务）v1 契约 |
...
| `schedule_record` | **打包 N 已落码 2026-09-26（docs/68 §2/§3；迁移 030 `schedules`，提交 `dae7c77`／装配 `e1d5869`）**：定时触发注册项，PK `(tenant_id, graph_id)`（`version`、`cron`、`enabled`、`created_at`、`last_fired_at`、`last_skipped_at`、`skip_count`）；**发布时派生**、只跑已发布钉版、UTC-only；REST 投影权威＝`scheduling/models.py::schedule_projection`（两档共用同一函数，逐键一致由 U886 断言而非承诺） | docs/68 §2.2 |
| `schedule_fire` | **打包 N 已落码 2026-09-26（docs/68 §1 D-5/§2.2；迁移 030 `schedule_fires`）**：槽位一次性认领账本，PK `(tenant_id, graph_id, slot_utc)`，`INSERT ... ON CONFLICT DO NOTHING` 的 rowcount 即互斥（照 docs/62 L2 同一条纪律）；**只记实际派发、无 outcome 列**，重叠跳过不写本表、不消耗槽位；时间列取 `TIMESTAMPTZ`（键列的唯一性不能依赖文本序列化）；内存档此表**易失**＝重启可重复派发一次，是契约不是缺陷 | docs/68 §2.2 |
...
>
> **调试断点持久化注记（docs/60 G4，2026-09-24 落码收口 c8f1172/b46e12b，零迁移/零后端产品改动）**：SerializedGraph 顶层允许**可选超集字段** `debugSettings?: { breakpoints?: PersistedBreakpoint[] }`，随图保存/加载往返、不参与编译拓扑。`PersistedBreakpoint = { nodeId: string, expression?: string(trim 非空), hitCount?: 正整数, logMessage?: string(trim 非空), onException?: true }`（空字段裁剪不落、空断点仅留 nodeId）。**后端零白名单**：GraphDSL（`graph/dsl.py`）无 `extra=forbid`，`parse_graph` 忽略未知顶层键、save/update_draft 存原始 dict、GET 原样返回，故 debugSettings 天然透传。前端 `graphSerializer.ts` 序列化/反序列化、`editorStore.loadGraph` 还原（节点已不存在的孤儿断点丢弃）、Editor 三处保存/编译全传 breakpoints。普通运行（/run、/run/stream）body 不带 debug 即不暂停，有黑盒测试锁死。团队共享/权限仍随 D27 缓做。
...
>
> **消息渠道余部立项（docs/58，2026-09-24，立项中未落码，部分取回 D24）**：①webhook 渠道开放 secret 并出站 HMAC-SHA256 签名——确定性紧凑 UTF-8 序列化（`ensure_ascii=False, separators=(',', ':')`），头 `X-Atlas-Timestamp`（UTC 秒）与 `X-Atlas-Signature: sha256=<hex>`，签名基串 `f"{ts}\n{raw}"`，无 secret 不发签名头（Atlas 自定义出站约定）；②IM 新增 `msgFormat: text|markdown`（缺省 text）与 `mentions: {userIds[], mobiles[], atAll}`（各 ≤20）——钉钉 markdown `{markdown:{title,text},at:{atMobiles,atUserIds,isAtAll}}`（正文须含 @标识）、企微 markdown `{markdown:{content}}` 内拼 `<@userid>`/`<@all>`（无 mentioned_* 字段；mobiles 仅 text）、飞书 markdown 落 post 富文本（title＋按行分段＋末段 `{tag:at,user_id}`，all 用 user_id="all"；mobiles 忽略）；③webhook/IM 的 to 放开 1-20 URL 数组，逐目标 SSRF（EgressDenied fail-fast）、单目标内沿用 docs/56 退避、per-URL DeliveryRecord、全成才写 _messages，任一投递失败 best-effort 发完其余后聚合抛错。零新依赖/零迁移/无新 REST；短信、IM 应用 OAuth/入站、模板 CRUD/多语言、飞书 interactive 卡片、AlertChannel 多目标配置化仍缓做（14 D24，不解除）。三家消息体以官方文档核对（open.dingtalk.com/document/development/robot-message-type、developer.work.weixin.qq.com/document/path/91770、open.feishu.cn/document/client-docs/bot-v3/add-custom-bot）。
>
...
>
> **静默编辑＋值班按日轮换注记（docs/60 G3，2026-09-24 落码收口 9dfc593/6ba5816/89cc9ec，迁移 026，部分取回 D28）**：①新增 `PUT /api/monitoring/silences/{id}`（administer）编辑静默——只改 `reason` 与到期（expires_at），created_at/rule_id/graph_id/created_by 范围不可变；不存在 404、已过期 409、viewer 403。②`monitoring_oncall` PG 表（迁移 026 ALTER，002 同步）加两列 `rotation_interval_days INT NULL`、`last_rotated_at TEXT NULL`（**落码偏差**：契约草拟 TIMESTAMPTZ，为与 024 同表 updated_at/created_at 统一 TEXT，存 UTC `YYYY-MM-DD`）；`PUT /api/monitoring/on-call` body 增可选 `rotationIntervalDays`（1-365，null/缺省关闭自动轮换）。纯函数 `maybe_auto_rotate(schedule, now)` 在 `get_oncall` 与告警命中取值班人的**读路径惰性**按 UTC 日界推进 index（while 跨整天、对成员数取模、更新 last_rotated_at），不引定时器、不做分布式锁（v1 接受多实例重复推进幂等、最坏多走一步）。内存档同形。前端静默 Popover 编辑（改原因/延长到期）＋值班条轮换间隔 InputNumber（留空关闭）与「每 N 天·下次日期」派生展示。真正定时器/多值班组/排班日历/分布式锁仍缓做 D28。
...
id: string                 # default-balanced | strict-sre | demo-lenient | custom-quickstart
name: string
description: string
tags: [string]
config: RuleConfig         # 完整规则配置（四段齐备，必须过 validate_rules）；GET 列表投影不含此字段
# GET /api/alert-rule-templates（read，投影不含 config）/ GET /api/alert-rule-templates/{id}（read，含 config，未知 404）
# 一键应用＝前端取 config 后复用 PUT /api/monitoring/rules（administer，全量替换），无新写端点
```
...
OperationDescriptor = {
  ...
  security:    [[str]]              # docs/44 立项；有效要求 OR-of-AND（operation 覆盖全局），[] 无需密钥
  skipped:     bool
