...
| `schedule_record` | **打包 N 立项 2026-09-26（docs/68 §2/§3，⬜ 待落码）**：定时触发注册项，PK `(tenant_id, graph_id)`（`version`、`cron`、`enabled`、`created_at`、`last_fired_at`、`last_skipped_at`、`skip_count`）；**发布时派生**、只跑已发布钉版、UTC-only | —（工程契约） |
| `schedule_fire` | **打包 N 立项 2026-09-26（docs/68 §1 D-5/§2.2，⬜ 待落码）**：槽位一次性认领账本，PK `(tenant_id, graph_id, slot_utc)`，`INSERT ... ON CONFLICT DO NOTHING` 的 rowcount 即互斥（照 docs/62 L2 同一条纪律）；**只记实际派发、无 outcome 列**，重叠跳过不写本表、不消耗槽位 | —（工程契约） |
