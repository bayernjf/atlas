...
>
> **⚠️ 状态更新（2026-09-25 第四次复审，docs/63）：上面那句「清单权威」已失效——本文件的结论链是 docs/29 → docs/34 → docs/63，上线判定与阻断清单**以 [docs/63](63-项目级上线复审-2026-09-25第三次.md) 为唯一最新事实源**（其 §0A 是最新一节，已撤回同日「工程侧就绪」的推进）。本文正文按留档规则原样保留、不追溯改写：其中的 P0/P1 若干项此后已立项并落码（docs/56–65），而 docs/63 §0A 新登记的 N1（prod 全新安装登录死锁）与 N2（真实店铺退款在图里跑不出来）本文并未涵盖——**不要从本文取当前判定**。
...
- **2026-09-22 上表装配缺口已解除（`b2ac85d`）**：为 [storage/pg.py](../src/atlas/storage/pg.py) 的 `PgUserStore` 补一个与内存版 `UserStore` 同构的显式 no-op `bind_session_store`。PG 档会话吊销本就在 `update`/`set_password` 内经 `PgBackend.session_store()` 单例现取（操作同一 `iam_sessions` 表），会话 PG 化由 `PgSessionStore` 承担（docs/30 §4），故无需像内存档外部持有引用；不是把会话存储进程内化，也不在 deps 加 backend 分叉。端到端用例 U269 的 skipif 同步补 `ATLAS_STORAGE_BACKEND=='pg'` 条件（内存/直连档正确 skip、仅整栈 PG 档执行）。实证：① `ATLAS_STORAGE_BACKEND=pg` 真实起 uvicorn，`GET /api/health` 返回 **HTTP 200 `{"status":"ok"}`**、日志 Application startup complete、无 AttributeError/Traceback；② U269 整栈 PG 档 **PASSED**；③ 后端内存档全量 **1051 passed/34 skipped**（与 docs/33 基线一致、零回归）；④ PG 直连套件（ATLAS_RUN_INTEGRATION=1＋DATABASE_URL、不设 backend，7 文件）**56 passed/1 skipped**，唯一 skip 即 U269。此为阻断 #1 之下**具体装配缺口**的解除，使整栈 PG 形态可启动；阻断 #1 总评（交付形态默认仍内存、PG 缺省不启用、备份/恢复与生产部署未走完）与其余五项阻断结论不变。
