...
> 状态：**✅ 2026-09-23 落码收口**（D19 wait 余部 + D28 告警 lifecycle 余部，部分取回、**不解除缓做条目**；零新依赖 / 零迁移 / 无 ADR / 零新 REST）。原子链见下方落码注记。

> **落码收口注记（2026-09-23，实测最终事实）**：六步原子序全部完成——① docs-only 立项 `67650ed`；② wait 上限放开 + duration jitter `76fa61c`（dsl 常量 MAX_WAIT_SECONDS 600→3600、MAX_EVENT_WAIT_SECONDS 3600→86400、新增 MAX_JITTER_SECONDS=300/MAX_EVENT_KEYS=8；loader duration static actual=planned+randint(0,jitter)、`jitter_rng` 经 run_graph/compile/_execute_subgraph 全链可注入、仅首次进入附 plannedDurationSeconds/jitterSeconds、resume 不二次抖动；U510–U519）；③ event 多事件 OR 竞速 + 跨重启 restore `dcf4626`（event_waits.py 重写：`request_any(event_keys=)`、`_Pending.event_keys`、signal 向 payload 内注入 matchedEventKey、list_pending 增 eventKeys、restore 多键幂等；dsl eventKeys 1-8 与 eventKey 互斥校验；loader event 分支 A–G 多键插值/挂载/帧 eventKeys/输出 matchedEventKey·eventKeys、单键统一走 request_any；api restore 传 event_keys；U520–U530）＋docs `b44b906`；④ 告警自动 recovery + lifecycle 限流 `2d53654`（record_run 一次健康即 resolve 该图 open 且 rule_id≠rollout_gate 内置告警、复用 last_seen 不加列、acknowledged/rollout_gate 不恢复、锁外发 recovery；AlertNotifier 增 lifecycle_min_interval_seconds=60/time_func、同 (alert,transition) 窗口内跳过返空 delivery、成败都计时、new 首条不限；U531–U536）＋docs `fb9eac3`；⑤ 前端面板 `2e86862`（l1 常量/校验、wait.schema、WaitConfig static 抖动 InputNumber＋event 单/多竞速 Radio＋eventKeys 动态列表 n/8、NodeConfig 类型；新增 7 个 L1 用例）＋handoff `5d089a3`；⑥ docs 收口（本注记 + 04/03/12/13/14/00/08/CHANGELOG/handoff）。
> **收口门（以实跑为准）**：后端全量内存档 **1630 passed / 61 skipped / 0 failed**（349s）；PG 直连集成（含 U402 多键 restore）test_storage_pg_integration.py **14 passed**；前端 **643 passed/2 skipped**、oxlint 0 error（6 既有 warning）、tsc+vite build 过。测试编号 **U510–U536**（后端）＋前端 7 个 docs/54 L1 用例。
> **落码细化（不超 §1 范围）**：① matchedEventKey 为契约增强——**单键信号也带**（signal_key=命中键、signal_token=首键，setdefault 首决不覆盖），凡断言 payload 精确等于输入的旧用例已补键；② 前端 WaitConfig 为 M4 起保留的手写面板（硬编码中文、未走 i18n t()），本批沿用其风格，新文案未抽 editor namespace（与 docs/33 Wait 未 i18n 化范围一致；全量 vitest 含 i18n 守护通过）；③ jitter 仅 duration static，dynamic/absolute/event 超时均不应用；④ lifecycle 限流状态进程内不持久化。
> **收口零回归发现并修复（2026-09-23，HTTP smoke 对 live :8000）**：**(a) fix(api)**：同步 `/api/graphs/{id}/run` 的运行期 wait 确定性失败（WaitNodeFailure：WAIT_TIMEOUT_FAILED / WAIT_EVENT_KEY_INVALID / WAIT_DURATION_INVALID / WAIT_ABSOLUTE_TIME_INVALID）原先在 run_saved_graph 记录失败后 re-raise、且无全局 handler，逃逸到 ASGI 顶层被 Starlette re-raise，uvicorn 随之关闭该 keep-alive 连接，失败运行后紧邻的列表/详情 GET 复用连接被 RST（Connection reset by peer；docs/47 起既有、机器高负载时序下稳定暴露）；新增 `@app.exception_handler(WaitNodeFailure)` 返回结构化 500 JSON（detail.code/message/nodeId）、不 re-raise、保持连接（SSE 后台异常路径不受影响）。**(b) smoke 同步 docs/53/54/B1 语义**：event_wait ⑥ 候选 run 状态 running→(running,suspended)（docs/53 起 event wait 挂起在所有后端置 suspended）、① payload 补 matchedEventKey（单键也带）；alert_notify ②/⑤ 同步 B1 lifecycle（普通与 rollout_gate 合并各发一条 merged、rollout_gate 健康运行不自动恢复），并新增 merged 60s 限流（注入可控时钟：投递→窗口内跳过→窗口外恢复）与 recovery（隔离 store：一次健康运行发一条、二次不重复）断言；dynamic_wait over-max 用例 601→3601（duration 上限 600→3600）。**(c)** `.smoke/event_wait_smoke.py` 自 docs/47 起漏 `git add -f`（.smoke/ 在 .gitignore，其余 5 个 smoke 均为强制入库），本次补入库；新增 `scripts/dev/d54_smoke_run.sh` 一键复跑三 smoke。HTTP smoke 终值：event_wait **19**、alert_notify **21**、dynamic_wait **21** 全绿；受影响后端针对性 85 passed；全量内存门 skipped 60→61 系新增 PG-only 多键测试在无 DATABASE_URL 内存门跳过（PG 门下 14 passed）。
> 原立项状态：2026-09-23 docs-only 立项（D19 wait 余部 + D28 告警 lifecycle 余部，部分取回、**不解除缓做条目**；零新依赖 / 零迁移 / 无 ADR）。
