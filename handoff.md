...
10. 🔧 **D9 BENCHMARK 实测（2026-09-14 重启，进行中）**：触发条件"引擎可跑后"已满足。落独立基准脚本（stdlib 零新依赖）测量 OODA 循环吞吐、Graph 编译延迟、退款端到端时延（规则路径）、Harness 调用开销，回填 BENCHMARK.md Results 首份基线（含 commit/环境注记）；LLM 决策延迟与记忆层读写随接入补测。
