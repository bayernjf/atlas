...
- **Python >= 3.11**（引擎与 Demo 后端，见 [atlas-docs/10-技术选型决策记录.md](atlas-docs/10-技术选型决策记录.md)）
...
代码骨架按 [atlas-docs/09-工程骨架与目录结构.md](atlas-docs/09-工程骨架与目录结构.md)：
...
- **文档是契约**：数据结构与接口签名以 `atlas-docs/03`（Schema）与 `12`（API）为基准，实现不得偏离；确需调整先走「改内容流程」（见 [atlas-docs/00-文档索引与治理.md](atlas-docs/00-文档索引与治理.md)）。
...
- Demo 验收对照 [atlas-docs/08-任务迭代计划.md](atlas-docs/08-任务迭代计划.md) 7.3 七条验收标准。
- 上线前过「四道门」：单元 → 集成 → 回放 → 端到端 + AI 评估（见 13 文档）。

## Branch & PR

- 分支策略：`feature/*`（开发）→ `dev`（集成/准生产）→ `main`（生产稳定版），详见 [atlas-docs/15-环境与分支策略.md](atlas-docs/15-环境与分支策略.md)。
