...
1. **唯一事实源**：`atlas-docs/` 下 01-08 为规格文档的唯一事实源；10-13 为工程化推导文档，与正文冲突时以 01-08 为准。
...
- `pyproject.toml` 依赖清单以 [atlas-docs/10-技术选型决策记录.md](atlas-docs/10-技术选型决策记录.md) §2 为准（Demo 依赖已就绪，版本号落码时解析）。
...
- 代码目录按 [atlas-docs/09-工程骨架与目录结构.md](atlas-docs/09-工程骨架与目录结构.md) 组织：`src/atlas/{engine,harness,graph,nodes,memory,skills,collaboration,web}/` + `tests/`。
...
代码未启动，暂无实测命令。落码后按 [atlas-docs/13-测试用例清单.md](atlas-docs/13-测试用例清单.md) 建立：
