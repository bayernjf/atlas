...
2. Group changes strictly by logical separation rules:
   ...
   - Separate config files (package.json, pyproject.toml, tsconfig, .env, vite.config) as chore/build commit
   - Separate test files (__tests__, *.test.ts, tests/) as test commit
